\justifying
We also instantiate the NC abstraction in interactive graphical user interface (GUI) environments with \ncguiworld{}.
...
The dataset includes two styles of random interaction, which we refer to collectively as GUIWorld Random: ``Random Slow'' and ``Random Fast'', plus a smaller set of supervised trajectories collected with Claude CUA~\citep{claudeComputerTool}.
...
For GUIWorld Random, our data-collection pipeline builds on the NeuralOS setup~\citep{rivard2025neuralos}: data is collected inside an Ubuntu~22.04 container running XFCE4 (Arc-Dark theme, Papirus icons) on a fixed 1024$\times$768 virtual display at 15~FPS, and we adopt the NeuralOS cursor-position representation, with task-specific adaptations for our setting.
